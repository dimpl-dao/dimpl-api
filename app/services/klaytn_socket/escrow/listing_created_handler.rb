    ...
    module Escrow
        class ListingCreatedHandler < ApplicationService
            ...
            def call
                ...
                if listing
                    listing.update!({hash: hash, deposit: deposit, remonstrable_block_interval: remonstrable_block_interval, price: price}) if listing.user_id == user.id
                else
                    listing = Listing.new({hash: hash, user_id: user.id, deposit: deposit, remonstrable_block_interval: remonstrable_block_interval, price: price})

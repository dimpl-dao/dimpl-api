class ImagePresignedUrlCreator < ApplicationService
    ...
    def create_blob(blob_args)
        ...
        extension = @params[:file][:content_type].split("/").last
        file_id = SecureRandom.uuid + "." + extension
        blob.update_attribute(:key, @key + file_id)
        return blob
    end
